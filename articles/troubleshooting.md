...
Most likely the wrong GDAL DLL was resolved first, or activation never
ran in this session.

- Run
  [`gdalraster.windows::activate_gdal_runtime()`](https://docs.jimbrig.com/gdalraster.windows/reference/activate_gdal_runtime.md)
  in a **fresh** R session, then load `gdalraster`.
- Inspect `Sys.getenv("PATH")` for competing GDAL installations (Rtools,
  conda/pixi, OSGeo4W) appearing before the bundle’s `bin/`.
- Confirm `gdalraster` was built by
  [`install_gdalraster()`](https://docs.jimbrig.com/gdalraster.windows/reference/install_gdalraster.md)
  against the bundle, not installed as the CRAN binary (which statically
  links Rtools’ GDAL).
...
fails loudly when the preload of `libgdal-*.dll` itself fails, so a
`LoadLibrary failure` surfacing only at
[`library(gdalraster)`](https://firelab.github.io/gdalraster/) usually
means the runtime was never activated in this session.

- Activate the runtime before
  [`library(gdalraster)`](https://firelab.github.io/gdalraster/) — or
  use
  [`gdalraster.windows::load_gdalraster()`](https://docs.jimbrig.com/gdalraster.windows/reference/load_gdalraster.md),
  which does both.
...
- Inspect the dependency tree from an
  ...
  `objdump -x <gdal_home>/bin/libgdal-*.dll | findstr "DLL Name"`.

A DLL can be “missing” even when CI verified the bundle: GitHub runner
images ship products that install DLLs into `System32` (e.g. the
Microsoft ODBC Driver for SQL Server, `msodbcsql17.dll`), so a
dependency on them resolves in CI but not on end-user machines. The
build disables such linkage (`GDAL_USE_MSSQL_ODBC=OFF`) and bundle
verification rejects known non-OS `System32` DLLs, but any import in the
...
process. The installer releases the current session’s own preloaded
runtime DLLs automatically, and refuses to run at all while `gdalraster`
...
- A `python.exe` must be discoverable on `PATH` for GDAL to embed an
  interpreter at all (the GDAL debug stream shows which python/libpython
  it loads).

Note: `validate_gpkg` treats the compiled `osgeo` bindings as optional;
the bundle intentionally omits them, which skips only tiled gridded
coverage content checks.

## CRS / projection errors despite GDAL loading

`GDAL_DATA` / `PROJ_DATA` are not pointing at the bundle’s `share/`
tree.
[`activate_gdal_runtime()`](https://docs.jimbrig.com/gdalraster.windows/reference/activate_gdal_runtime.md)
sets all three (`GDAL_DATA`, `PROJ_LIB`, `PROJ_DATA`); check whether
something later in your startup overwrites them.

## Download failures during `install_gdal_runtime()`

On machines without network access (or behind strict proxies), install
from a manually transferred release asset:
...
- Reinstall the runtime (`install_gdal_runtime(overwrite = TRUE)`), then
  rebuild `gdalraster` against it
  ([`install_gdalraster()`](https://docs.jimbrig.com/gdalraster.windows/reference/install_gdalraster.md))
  — the previous `gdalraster.dll` is bound to the previous GDAL’s
  ABI/import names.
- Never assume the GDAL SONAME is stable across versions; everything in
  this package discovers `libgdal-*.dll` by glob, and your own scripts
  should too.
