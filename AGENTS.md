...
Setup, sitrep, verify, and update default to `.libPaths()[1]`. Pass
`lib` for a custom library, or `isolated = TRUE` for the package-managed
isolated library under
`tools::R_user_dir("gdalraster.windows", "data")/library`.
...
9.  Default installs target the user library; isolated installs are
    opt-in.
