...
- `collect_dlls.sh`: assembles the self-contained runtime bundle (`bin`,
  `include`, `lib`, `share`, `python`) and enforces dependency closure —
  any non-Windows-system DLL left unresolved fails the build.
...
- `include/` headers and `lib/` import libraries for compiling
  `gdalraster` against the bundle
