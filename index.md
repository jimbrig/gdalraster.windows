...
Install the package, then build a self-contained `gdalraster` into the
...
Pass `lib` for a custom library, or `isolated = TRUE` for a
package-managed library that does not replace `.libPaths()[1]`.
